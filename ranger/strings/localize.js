...
const localize = {
    ...
    fileNameMap : {
        en : "strings/ranger_English.json",
        es : "strings/ranger_Spanish.json",
        de : "strings/ranger_German.json",
        fr : "strings/ranger_French.json",
        it : "strings/ranger_Italian.json",
    },
    ...
    getString : function(iID, ...theArgs) {
        const theRawString = eval(`DG.plugins.ranger.${iID}`);
        let out = "";
        if (theRawString) {
            out = replaceSubstrings(theRawString, ...theArgs);
        } else {
            const theDefaultString = eval(`this.defaultStrings.ranger.${iID}`);
            if (theDefaultString) {
    ...
    setStaticStrings: async function () {
        ...
        const theStaticStrings = DG.plugins.ranger.staticStrings;
