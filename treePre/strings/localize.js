...
const localize = {
    ...
    fileNameMap : {
        en : "strings/treePre_English.json",
        es : "strings/treePre_Spanish.json",
        de : "strings/treePre_German.json",
        fr : "strings/treePre_French.json",
        it : "strings/treePre_Italian.json",
    },
    ...
    getString : function(iID, ...theArgs) {
        const theRawString = eval(`DG.plugins.treePre.${iID}`);
        let out = "";
        if (theRawString) {
            out = replaceSubstrings(theRawString, ...theArgs);
        } else {
            const theDefaultString = eval(`this.defaultStrings.treePre.${iID}`);
            if (theDefaultString) {
    ...
    setStaticStrings: async function () {
        ...
        const theStaticStrings = DG.plugins.treePre.staticStrings;
