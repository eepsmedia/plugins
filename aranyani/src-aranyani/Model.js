...
class Model extends Object {
    ...
    async checkForEndGame() {
        ...
        if (tReasonObject.end) {
            this.theGame.gameState = aranyani.constants.kEndedString;      //  side effect; change the game
        } else {
    ...
    /**
     * In case not all players are playing; aranyani can proceed when somebody doesn't move.
     *
     * @returns {[]}
     */
    playingPlayers() {
    ...
    getCurrentSituation() {     //  todo: be SURE this shouldn't be async!
        ...
        if (this.theGame.gameCode) {
            ...
            tPlayingPlayers.forEach((p) => {
                ...
                if (p.playerState !== aranyani.constants.kSellingString) {
            ...
            if (this.theGame.gameState === aranyani.constants.kInProgressString) {
                return {
                    OK: ok,
