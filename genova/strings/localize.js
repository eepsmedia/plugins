...
function replaceSubstrings(originalString, ...substitutions) {
    ...
    const resultString = originalString.replace(regex, (match, index) => {
        ...
        return (substitutions[substitutionIndex] === 0) ?
            0 :
            substitutions[substitutionIndex] || match; // Use substitution or original match if not available
    });
...
const localize = {
    ...
    fileNameMap : {
        en : "strings/genova_English.json",
        es : "strings/genova_Spanish.json",
        de : "strings/genova_German.json",
        fr : "strings/genova_French.json",
        it : "strings/genova_Italian.json",
    },
    ...
    getString : function(iID, ...theArgs) {
        const theRawString = eval(`DG.plugins.genova.${iID}`);
        let out = "";
        if (theRawString) {
            out = replaceSubstrings(theRawString, ...theArgs);
        } else {
            const theDefaultString = eval(`this.defaultStrings.genova.${iID}`);
            if (theDefaultString) {
    ...
    setStaticStrings: async function () {
        ...
        const theStaticStrings = DG.plugins.genova.staticStrings;
