...
const localize = {
    ...
    fileNameMap: {
        en: "strings/lottini_English.json",
        es: "strings/lottini_Spanish.json",
        de: "strings/lottini_German.json",
        fr: "strings/lottini_French.json",
        it: "strings/lottini_Italian.json",
    },
    ...
    getString: function (iID, ...theArgs) {
        const stringLoc = `DG.plugins.lottini.${iID}`;
        const theRawString = eval(stringLoc);
        let out = "";
        if (theRawString) {
            out = replaceSubstrings(theRawString, ...theArgs);
        } else {
            const theDefaultString = eval(`this.defaultStrings.lottini.${iID}`);
            if (theDefaultString) {
    ...
    setStaticStrings: async function () {
        ...
        const theStaticStrings = DG.plugins.lottini.staticStrings;
